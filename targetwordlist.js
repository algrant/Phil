...
openDefaultTargetlist("TARGET-LIST.txt");

//____________________
// F U N C T I O N S

function addToTargetlist(newWords) {
  for (i = 0; i < newWords.length; i++) {
    const word = newWords[i].trim().toUpperCase();
    if (word.length < targetList.length) { // Make sure we don't access outside the targetList array
      targetList[word.length].push(word);
    }
  }
}

function sortTargetlist() {
  for (let i = 3; i < targetList.length; i++) {
    targetList[i].sort();
  }
}

function openTargetlist() {
  document.getElementById("open-targetList-input").click();
}

function openTargetlistFile(e) {
  targetList = [
    [], [], [], [], [],
    [], [], [], [], [],
    [], [], [], [], [], []
  ...
  reader.onload = function(e) {
    ...
    addToTargetlist(words);
    sortTargetlist();
    removeTargetlistDuplicates();
    invalidateSolverTargetlist();
  };
  reader.readAsText(file);
}

function openDefaultTargetlist(url) {
  ...
  textFile.onreadystatechange = function() {
    if (textFile.readyState === 4 && textFile.status === 200) {  // Makes sure the document is ready to parse, and it's found the file.
      ...
      addToTargetlist(words);
      sortTargetlist();
      console.log("Loaded default targetList.")
    }
  }
  textFile.send(null);
}

function removeTargetlistDuplicates() {
  for (let i = 3; i < targetList.length; i++) {
    if (targetList[i].length >= 2) {
      for (let j = targetList[i].length - 1; j >0; j--) {
        if (targetList[i][j] == targetList[i][j - 1]) {
          targetList[i].splice(j, 1);
        }
      }
    }
  }
...
function matchFromTargetlist(word) {
  const l = word.length;
  const actualLettersInWord = word.replace(/-/g, "").length;
  if (actualLettersInWord < l) { // Only search if word isn't completely blank or filled
    word = word.split(DASH).join("\\w");
    const pattern = new RegExp(word);
    let matches = [];
    for (let i = 0; i < targetList[l].length; i++) {
      if (targetList[l][i].search(pattern) > -1) {
        matches.push(targetList[l][i]);
      }
    }
    return matches;
  } else {
...
function updateTargetMatchesUI() {
  let acrossMatchList = document.getElementById("target-across-matches");
  let downMatchList = document.getElementById("target-down-matches");
  acrossMatchList.innerHTML = "";
  downMatchList.innerHTML = "";
  const acrossMatches = matchFromTargetlist(current.acrossWord);
  const downMatches = matchFromTargetlist(current.downWord);
  for (i = 0; i < acrossMatches.length; i++) {
    let li = document.createElement("LI");
    li.innerHTML = acrossMatches[i].toLowerCase();
    li.className = "";
    // li.addEventListener('click', printScore);
    li.addEventListener('dblclick', fillGridWithTargetMatch);
...
function fillGridWithTargetMatch(e) {
  const li = e.currentTarget;
  const fill = li.innerHTML.toUpperCase();
  const dir = (li.parentNode.id == "target-across-matches") ? ACROSS : DOWN;
